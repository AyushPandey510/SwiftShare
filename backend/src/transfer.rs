...
impl TransferEngine {
    ...
    async fn handle_connection(
        ...
        info!("Starting transfer {} for file: {}", transfer_id, request.filename);
    ...
    async fn handle_file_transfer(
        ...
        if metadata.len() != request.size {
            error!("File size mismatch: expected {}, got {}", request.size, metadata.len());
    ...
    pub async fn send_file(&self, target_addr: SocketAddr, file_path: PathBuf) -> Result<Uuid> {
        ...
        let handle = tokio::spawn(async move {
            if let Err(e) = Self::send_file_data(target_addr, file_path, transfer_id, request, transfers_clone, config).await {
    ...
    pub async fn cleanup_completed_transfers(&self, max_age_hours: u64) -> Result<usize> {
        ...
        
        {
            ...
            transfers.retain(|_, progress| {
                if progress.status == TransferStatus::Completed || progress.status == TransferStatus::Failed {
